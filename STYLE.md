...
together, or the other way round. `NoteEditorStyle` owns the ramp and the
ratios and hands the editor page whole-point values as CSS variables, so the
page never holds a design number of its own.
