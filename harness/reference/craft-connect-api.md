...
| `GET /documents` | `{items:[{id,title}]}`. Titles may be empty strings. Unfiltered it returns ALL documents (observed 654 in the live space) — fetch on demand, never at launch. |
| `POST /documents` | `{documents:[{title}]}` with no destination creates EMPTY in `unsorted` (verified live 2026-09-05: throwaway "CCP probe — delete me" → 200 `{items:[{id,title,clickableLink}]}`); content is a separate `POST /blocks`. `clickableLink` is a `craftdocs://` deep link — display-only, never stored as the mapping. |
| `DELETE /documents` | `{documentIds:[…]}` → 200 `{items:[ids]}`. SOFT-delete into trash (verified live: the probe left the list and appeared under `?location=trash`), never a hard erase. A trashed doc still answers `GET /blocks` with 200 and its full content (verified live 2026-09-06) — trash membership via `GET /documents?location=trash` is the only gone-signal, never the fetch status. |
