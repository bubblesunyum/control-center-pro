...
| `DELETE /documents` | `{documentIds:[…]}` → 200 `{items:[ids]}`. SOFT-delete into trash (verified live: the probe left the list and appeared under `?location=trash`), never a hard erase. |
