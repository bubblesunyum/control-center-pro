// SPDX-License-Identifier: GPL-3.0-or-later
// Copyright (C) 2026 Control Center Pro contributors

// Posts real keyboard events at the focused app, for the editor gate.
//   drive toggle | text:<chars> | key:<keycode>[:shift|cmd|cmdshift] | wait:<ms> ...
import CoreGraphics
import Foundation

let source = CGEventSource(stateID: .hidSystemState)

func fail(_ message: String) -> Never {
    FileHandle.standardError.write("\(message)\n".data(using: .utf8)!)
    exit(2)
}

func press(_ code: CGKeyCode, flags: CGEventFlags = [], unicode: String? = nil) {
    for down in [true, false] {
        guard let event = CGEvent(keyboardEventSource: source, virtualKey: code, keyDown: down) else { continue }
        event.flags = flags
        if let unicode {
            let utf16 = Array(unicode.utf16)
            event.keyboardSetUnicodeString(stringLength: utf16.count, unicodeString: utf16)
        }
        event.post(tap: .cghidEventTap)
        usleep(12_000)
    }
}

for action in CommandLine.arguments.dropFirst() {
    let parts = action.split(separator: ":", maxSplits: 1).map(String.init)
    switch parts[0] {
    case "toggle":
        press(50, flags: .maskCommand) // the panel's cmd-` shortcut
    case "text":
        for character in parts.count > 1 ? parts[1] : "" {
            if character == " " { press(49) } else { press(0, unicode: String(character)) }
        }
    case "key":
        let fields = parts[1].split(separator: ":")
        let names = fields.count > 1 ? String(fields[1]) : ""
        var flags: CGEventFlags = []
        if names.contains("shift") { flags.insert(.maskShift) }
        if names.contains("cmd") { flags.insert(.maskCommand) }
        guard let code = CGKeyCode(fields[0]) else { fail("bad keycode in \(action)") }
        press(code, flags: flags)
    case "wait":
        guard parts.count > 1, let ms = Int(parts[1]) else { fail("bad wait in \(action)") }
        usleep(useconds_t(ms * 1000))
    default:
        FileHandle.standardError.write("unknown action \(action)\n".data(using: .utf8)!)
    }
}
